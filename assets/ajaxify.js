...
ajaxify = function(settings) {
    ...
    if (moreURL.length){
      $.ajax({
        ...
        success: function(data) {
          ...
          if ($('.order-form-show-options').length > 0) {
            $('.order-form-show-options').unbind().on('click', function(e){
              var $this = $(this);
              var $form = $this.closest('.order-item');
              var $variantForm = $form.next('tr.order-variant-items');
  ...
  $.endlessScroll = function() {
    ...
    $(window).on( action, function() {
      ...
      moreURL = $(contentContainer).last().find(linkElem).attr('href');
      pageType = 'endlessScroll';
      $(linkElem).text(textChange);
      if ($(contentContainer).last().find(linkElem).length){
